...
타겟은 TryHackMe Kenobi room이 띄워 주는 원격 호스트(이 세션에서는 `10.49.185.92`, 세션마다 재할당됩니다)로, OpenVPN 터널 너머에 있습니다. 공격 측은 VMware의 Kali이고 `tun0`로 THM 망에 붙였습니다. 목표는 kenobi user 셸을 거쳐 root 셸(`uid=0`)과 `/root` 플래그까지 확인하는 것입니다.
...
80/tcp    open  http        Apache httpd 2.4.18   (robots.txt disallow: /admin.html)
...
마운트 지점 `/mnt/kenobi`와 키를 내려 둘 디렉터리 `~/mma`를 먼저 만들어 둬야 했습니다. 디렉터리가 없으면 `mount`가 "mount point does not exist"로 실패하는데, 이 실패를 놓치면 마운트가 걸리지 않은 빈 디렉터리를 그대로 뒤지게 되고 `/mnt/kenobi/tmp/id_rsa`가 없다고 나옵니다. 앞서 FTP에서 `250 Copy successful`을 받았는데도 마운트 지점에서 키가 안 보였던 것이 이 선행 단계 누락 때문이었습니다. 복사 목적지가 원격 `/var/tmp/id_rsa`이므로, `/var`를 `/mnt/kenobi`에 걸면 로컬 경로는 한 단계 안쪽인 `/mnt/kenobi/tmp/id_rsa`입니다.
