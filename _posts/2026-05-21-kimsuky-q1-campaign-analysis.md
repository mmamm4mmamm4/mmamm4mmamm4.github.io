---
title: 1분기 DPRK Operation Kimsuky 분석
...
| **C2 채널**    | 자체 서버<br>(hxxps://nelark[.]icu) | GitHub 저장소<br>(정상 서비스 악용)          | 자체 IP (103.67.196[.]25)   | 정상 서버 악용(yespp[.]co[.]kr) + GitHub OAuth |
...
LNK 파일(1.pdf.lnk) 실행 시 미끼 PDF를 추출하여 피해자에게 정상 문서로 보여주고, 시작 프로그램 폴더에 OneDrive.lnk 파일을 생성하여 지속성을 확보합니다. 동시에 C2 서버(hxxps://nelark[.]icu/xftaswx/res/bb.php)에서 추가 스크립트를 가져와서 실행합니다. 이후, 다양한 C2 통신 및 스크립트 코드를 수행하게 됩니다.
...
| 2단계 | `curl` | `hxxp://103.67.196[.]25/conf.dat` → `C:\Users\Public\Music\ant.vbe`로 저장 | C2 서버에서 VBE 다운로드 |
